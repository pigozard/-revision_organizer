...
The active record pattern is a pattern found in software that stores its data in relational databases. […] The interface of an object conforming to this pattern would include functions such as [ save , destroy ], plus [instance variables] that correspond […] to the columns in the underlying database table.
Wikipedia
...
You need to add tables, the one you figured out with kitt.lewagon.com/db
...
Active Record helps us to write schema migration with ruby, not SQL.


Run the migrations


Active Record figures out which migration to run on your schema (the ones not already run).
...
Remember the convention
 ...
 Model class name: UpperCamelCase, singular form (mapped to 1 row) Rails is full of Convention over Configuration.
Unleash the magic
...
Calling all - a class method - on your model gives you an ActiveRecord::Relation (similar to Array ) of all your records for this model. Elements of this ‘array’ are instances
of the model class Restaurant .
...
As for the all class method, where returns an ActiveRecord::Relation (similar to Array ) of class model instances.
...
Growth Hacking
Web scrapping (nokogiri) or API calls (rest-client)
Fetch GitHub repos from @lewagon



Browser extension to install: JSON Formatter on Chrome.
...
Your turn!
 Playboard	A Bookmarks	 Needs attention	 Rehearse
...
On crée une migration quand on a besoin de modifier le schéma de notre base de données (ajouter/supprimer une table, ajouter/supprimer une colonne, etc.).
C’est un changement de la structure de la base de données, qui n’a pas d’impact sur les données. Play this one again


 DB (ActiveRecord Basics)
Comment obtient-on un enregistrement spécifique d’un modèle Active Record ?


En appelant .find(id) sur ton modèle !
...
Si on n’a pas l’id de l’enregistrement que l’on cherche, on peut également appeler .find_by(attribut : valeur) :
...
Imaginons que tu aies déjà créé la base de données avec une table doctors (docteurs), comment ajouterais-tu un nouveau docteur dans ta base de données ?


En créant une instance de Doctor (docteur) et en appelant la méthode #save dessus :


Ou directement avec la méthode de classe create:

...
Play this one again
 ...
 DB (ActiveRecord Basics)
Tu crées une nouvelle migration. Quelle commande Terminal dois-tu utiliser pour l’exécuter ?
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment obtient-on le nombre d’instance d’un modèle Active Record donné ?


En appelant .count sur ton modèle !
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment obtient-on tous les docteurs ayant une spécialité donnée ?
...
On peut également passer une string en argument à la méthode de classe .where:
...
DB (ActiveRecord Basics)
Quelle est la requête SQL générée par Doctor.all ?
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment les tables de base de données doivent-elles être nommées pour Active Record ?

	A



Le nom de ta table doit être pluriel en lower_snake_case. Par exemple : sports_cars, doctors, restaurants
Le mapping magique 1:1 d’Active Record entre les enregistrements de ta base de données et les instances de tes modèles
dépend complètement de cette convention ! Play this one again


 DB (ActiveRecord Basics)
Quelle est la requête SQL générée par Doctor.find_by(age: 42, specialty: 'Dentist') ?
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment les modèles Active Record doivent-ils être nommés ?
...
Attention, ceci supprimera ton schéma de base de données (structure) ainsi que tes données ! Play this one again


 DB (ActiveRecord Basics)
Quelle tâche rake dois-tu exécuter dans ton terminal pour créer ta base de données ?
...
Active Record est un patron de conception (« design pattern » en anglais), qui stocke tes objets dans une base de données relationnelle.
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Quelle est la différence entre ::find_by et ::where ?

	A
::find_by retourne le premier enregistrement qui remplit la condition, tandis que ::where retourne tous les enregistrements qui remplissent la condition, dans un array.
N’oublie pas que ::where retourne toujours un array (même s’il n’y qu’un seul enregistrement ou aucun), et que
::find_by retourne une instance ou nil ! Play this one again

 DB (ActiveRecord Basics)
Complète la migration suivante pour ajouter une colonne âge (age) à la table doctors (docteurs).
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment obtient-on toutes les instances d’un modèle Active Record donné ?


En appelant .all sur ton modèle !
...
Play this one again
 ...
 DB (ActiveRecord Basics)
Par quoi le nom d’un fichier de migration doit-il toujours commencer ?


Un timestamp (« horodatage ») (format : YYYYMMDDHHMMSS) ! Par exemple :
db/migrate/20140725164644_create_restaurants.rb

	A


Le timestamp est nécessaire pour garder une trace des migrations qui doivent être exécutées lors de l’exécution de la commande rake db:migrate. Il permet également de classer les fichiers de migration par ordre chronologique dans le projet.
Play this one again
 ...
 DB (ActiveRecord Basics)
Comment obtient-on la première instance d’un modèle Active Record ?

	A
En appelant .first sur le modèle.
...
suivant : yyyymmddhhmmss_nom_de_la_migration.rb . Le timestamp dans le fichier est très important : il permet à rake db:migrate de savoir quelles migrations n’ont pas encore été exécutées.

Migration pour créer une table
Écris le code dans 20141025152200_create_posts.rb pour créer la table posts . Ta table posts doit contenir les colonnes suivantes :
  une string title
  une string url
...
Migration pour actualiser une table
Retourne au cours et lis la documentation sur les migrations Active Record. Tu verras comme il est facile d’effectuer des migrations sur Rails. Mais on n’a pas encore abordé Rails ;) alors tu vas devoir créer tes fichiers de migration manuellement.
